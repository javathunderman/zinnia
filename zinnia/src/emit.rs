...
pub fn emit(ast: &Spanned<Expr>) -> Result<ir::Context, calyx_utils::Error> {
    ...
    let mut comb_ns: frontend::NamespaceDef = frontend::NamespaceDef::construct(&Some(
        "../../838l-build-chain/calyx/primitives/memories/comb.futil".into(),
    ))?;
    let mut std_ns: frontend::NamespaceDef = frontend::NamespaceDef::construct(&Some(
        "../../838l-build-chain/calyx/primitives/core.futil".into(),
    ))?;
    ...
    for ext in comb_externs {
        ctx.lib
            .add_extern_primitive("../memories/comb.futil".into(), ext);
    }
